...
int Count (string text, char let)       // Задал метод для переборки слова
{
    ...
    {
        if (a[j] == let) b++;            // проверяю условие: если j-буква в слове соответствует введенной букве, то количество искомых букв увеличивается
    }
...
for (index = 0; index < city.Length; index++)          // делаю переборку заданного массива на поиск слов с заданной буквой
{
    if (city[index].Contains(letter.ToLower()))         // если буква есть в слове, то перехожу дальше
    {
        ...
        Console.WriteLine(city[index] + " " + max);
    } 
    // else Console.WriteLine("нет такой буквы");
}
